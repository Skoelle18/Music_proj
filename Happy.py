...
main_instruments = {'acoustic_guitar': 24, 'bright_acoustic_piano': 1, 'celesta': 8}
...
# Scale (corrected label: C major scale)
scale = [60, 62, 64, 66, 67, 69, 71, 72]  # C Lydian scale
...
# Generate background melody with harmonies
...
def add_percussion(track, bars=16):
    ...
    for bar in range(bars):
        for beat in range(4):
            ...
            track.append(Message('note_on', note=note, velocity=50, time=note_time, channel=channels['percussion']))
...
def add_notes(track, channel, melody, is_main=False):
    ...
    for i, note in enumerate(melody):
        velocity = random.randint(90, 120) if is_main else random.randint(40, 60)
